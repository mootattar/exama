...
export default function DashboardPage() {
  ...
  const averageScore = results.length > 0 
    ? Math.round(results.reduce((acc, result) => acc + (result.score / result.totalPoints) * 100, 0) / results.length)
    : 0;
              ...
              {recentExams.length > 0 ? (
                    ...
                    <div key={exam.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          {exam.questions.length} questions • {exam.timeLimit || 'No'} time limit
                        </p>
                      </div>
                ...
                <div className="text-center py-8">
                  ...
                  <Button asChild className="mt-4">
                    <Link href="/dashboard/new-exam">Create Your First Exam</Link>
                  </Button>
                </div>
              )}
            </CardContent>
                ...
                <div className="space-y-4">
                  {recentResults.map((result) => (
                    <div key={result.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-medium truncate">{result.examTitle}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Button asChild className="h-auto p-6 flex-col space-y-2">
              <Link href="/dashboard/new-exam">
                ...
                <span className="text-xs opacity-80">Start building your exam</span>
              </Link>
            </Button>
